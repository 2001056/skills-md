...
> 아래 예시는 설명의 편의를 위해 이커머스 도메인을 사용하지만, 동일한 원칙과 템플릿을 SaaS·핀테크·헬스케어·물류·소셜 등 **모든 서비스 기획에 그대로 적용**합니다.
...
예시 (SaaS — 멤버 초대):
```
Given 무료 플랜 팀의 관리자가 팀 설정 페이지에 있을 때
When 멤버 초대 버튼을 클릭하면
Then "무료 플랜은 최대 3명까지 가능합니다" 메시지가 표시되고 초대가 발송되지 않는다
```
...
> 실제 서비스에 맞게 역할명(뷰어/에디터/오너 등)과 기능명을 도메인에 맞게 교체합니다.
