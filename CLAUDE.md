...
| `/dev-investigate` | 조사 단계 오케스트레이터 | 재현 → 경쟁 가설 3개+ → 인과 사슬·재검증 → 검수 (단계 축소 없음) |
...
6. 영단어 신호는 `\bAPI\b` 처럼 `\b` 로 감싸면 된다 — `_kr_boundary()` 가 "API에", "DB로" 같은 **조사 결합**을 통과시키도록 자동 보정한다 (한글도 `\w` 라서 순정 `\b` 는 조사 앞에서 깨짐)
7. 버그 조사(investigate) 신호는 **신고 어법**만 — 바닥 명사(`에러`, `오류`, `실패`)를 그대로 넣으면 "에러 상태 UI", "실패 시 재시도" 같은 요구사항이 버그로 오탐된다. `(에러|오류)\s*...(나|발생|뜨)` 꼴로 동사를 붙일 것
8. README 빠른 시작 예시 문장은 `tests/test_gates.sh` 10번에 묶여 있다 — 예시를 바꾸면 테스트도 같이 바꿀 것
